...
module.exports = class MongoDBSort {
	...
	static parseSortingParams(order) {
		...
		if(this.shouldAddIdSort(parsedSortParams)) {
	...
	static shouldAddIdSort(parsedSortParams) {
		const sortParamsFields = Object.keys(parsedSortParams);
		return !sortParamsFields.some(field => field.toLocaleLowerCase().startsWith('date') || field.toLocaleLowerCase().endsWith('date'));
	}
