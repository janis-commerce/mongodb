...
class MongoDB {
	...
	async get(model, params = {}) {
		...
		try {
			...
			const res = await this.mongo.makeQuery(model, collection => collection
				.find(ObjectIdHelper.ensureObjectIdsForWrite(model, filters))
				.sort(order)
	...
	async save(model, item = {}) {
		...
		try {
			...
			const { _id, ...setData } = mongoItem; // Remove _id to prevent upsert errors
	...
	async multiSave(model, items) {
		...
		const bulkItems = items.map(item => {
			...
			const { _id, dateCreated, dateModified, ...setData } = mongoItem; // Remove _id to prevent upsert errors
