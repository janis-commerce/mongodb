			...
			if(rawResponse) {
				return {
					...
					operations: operations.map((operation, index) => {
						// Check if this operation had any write errors
						const operationErrors = result.getWriteErrors()?.filter(error => error.index === index) || [];
		...
		try {
			...
			const res = await this.mongo.makeQuery(model, collection => collection
				.findOneAndUpdate(filters, {
					...
					upsert: false,
					returnNewDocument: true, // 'returnNewDocument: true' return the updated document
					includeResultMetadata: true, // driver v6 defaults to returning the document only
					...getCommentOption()
				}));
