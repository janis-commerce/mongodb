...
module.exports = class MongoDB {
	...
	async getPaged(model, params, callback) {
		...
		const getPagedParams = params || {};

		const sort = MongoDBSort.parseSortingParams(getPagedParams.order);
		const batchSize = getPagedParams.limit || this.config.limit;

		const filters = MongoDBFilters.parseFilters(ObjectIdHelper.ensureObjectIdsForWrite(model, getPagedParams.filters || {}), model);
		const project = MongoDBProject.parse(getPagedParams);
