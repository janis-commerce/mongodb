...
const MongoAggregate = require('./mongodb-aggregate');
...
class MongoDB {
	...
	async get(model, params = {}) {
		...
		if(params.group)
			return this._group(model, params);

		return this._get(model, params);
	}

	/**
	 * Get simple data from mongodb database
	 *
	 * @param {Model} model Model instance
	 * @param {Object} params parameters (limit and filters)
	 * @returns {Array} mongodb get result
	 */
	async _get(model, params = {}) {
	...
	/**
	 * Get grouped data from mongodb database
	 * @param {Model} model Model instance
	 * @param {Object} params parameters (group, limit and filters)
	 * @returns {Array} mongodb get result
	 */
	async _group(model, params = {}) {
		try {

			if(!params.group)
				throw new Error('"group" field is required for get grouped data');

			const { limit, filters, group: { fields, by } } = params;
			const mongoAggregate = new MongoAggregate(model);
			const aggregate = mongoAggregate
				.group(fields, by)
				.limit(limit)
				.having(filters);
	...
	async getIndexes(model) {
		...
		try {
			...
			return IndexesHelper.format(indexes);
	...
	async createIndexes(model, indexes) {
		...
		if(!Array.isArray(indexes))
			throw new MongoDBError('Invalid indexes: Should be an array of indexes', MongoDBError.codes.INVALID_INDEX);

		indexes.forEach(index => IndexesHelper.validate(index));
