...
class MongoDB {
	...
	constructor(config) {
		...
		this.config = {
			host: config.host ? config.host.replace('mongodb://', '') : '',
	...
	*/
	...
	/**
	* Generates and appends the id field into the fields if already doesn't have it.
	* @param {Object} fields item fields
	*/
	prepareFields(fields) {
		if(!fields._id && typeof fields.id === 'undefined') // Se usa typeof porque fields.id puede ser 0
			return;

		if(!fields._id && typeof fields.id !== 'undefined') {
	...
	async save(model, item = {}) {
		...
		if(item._id) // Previene errores en el upsert
			delete item._id;
