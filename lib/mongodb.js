...
/**
 * @typedef {object} MultiUpdateOperationResult
 * Describes how a single update operation ended in a multiUpdate() detailed response.
 *
 * @property {number} index - The position of the operation in the operations array received by multiUpdate().
 * @property {object} filter - The filter of the operation.
 * @property {object} data - The data of the operation.
 * @property {object} [options] - The options of the operation.
 * @property {boolean} success - False when the server reported a write error for this operation.
 * @property {Array<object>} errors - The write errors reported for this operation.
 */

/**
 * @typedef {object} MultiUpdateRawResponse
 * The detailed response of multiUpdate() when the rawResponse option is used.
 *
 * @property {boolean} success - False when the bulk write reported write errors or a write concern error.
 ...
 * @property {Array<object>} writeErrors - The write errors reported by the server, indexed by operation.
 * @property {Array<object>} writeConcernErrors - The write concern error reported by the server, if any.
 * @property {MultiUpdateOperationResult[]} operations - The outcome of every received operation.
 */
...
const { MongoWrapper, MongoBulkWriteError, ObjectId } = require('./mongodb-wrapper');
...
const { getCommentOption } = require('./helpers/operation-comment');
...
module.exports = class MongoDB {
	...
	async distinct(model, params = {}) {
		...
		try {
			...
			if(process.env.MONGODB_DEBUG)
				logger.info(`distinct() ${model.constructor.table} - filters ${JSON.stringify(filters)}`);

			const res = await this.mongo.makeQuery(model, collection => collection.distinct(key, filters, {
				...params.readPreference && { readPreference: params.readPreference },
				...getCommentOption()
			}));
	...
	async getPaged(model, params, callback) {
		...
		try {
			...
			/* istanbul ignore next */
			if(process.env.MONGODB_DEBUG)
				logger.info(`getPaged() ${model.constructor.table} - filters ${JSON.stringify(filters)} - sort ${JSON.stringify(sort)}`);
	 ...
	 * @param {object} options An additional options for the save operation
	...
	async save(model, item, setOnInsert, options = {}) {
		...
		try {
			...
			const operationGroupedValues = this.groupByWriteOperation(setData, {
				...!options.skipAutomaticSetModifiedData && {
			...
			const res = await this.mongo.makeQuery(model, collection => collection
				.findOneAndUpdate(filters, operationGroupedValues, {
					upsert: true,
					includeResultMetadata: true, // driver v6 defaults to returning the document only
					...getCommentOption()
				}));
	...
	async multiInsert(model, items, options = {}) {
		...
		try {
			...
			*/
			...
			indexesAndIds = Object.entries(res.insertedIds)
				.map(([index, _id]) => ({ index: Number(index), _id }));

		} catch(err) {

			const isDuplicateError = err.message.includes('E11000 duplicate key error collection');
			if(!isDuplicateError || options.failOnDuplicateErrors)
				throw new MongoDBError(err, MongoDBError.codes.MONGODB_INTERNAL_ERROR);

			logger.error(`MongoDB Error multiInsert(): ${err.message}`);

			// Driver v5+ removed the raw `.result.result`. BulkWriteResult.insertedIds is now
			// an object keyed by insertion index ({ '0': ObjectId, ... }) and writeErrors lives
			// directly on the MongoBulkWriteError (single WriteError or array).
			const insertedIdsMap = err.result?.insertedIds || {};
			const writeErrors = [].concat(err.writeErrors || []);

			indexesAndIds = Object.entries(insertedIdsMap)
				.map(([index, _id]) => ({ index: Number(index), _id }));
		...
		return indexesAndIds
			.filter(({ index }) => !indexesWithError.includes(index))
			.map(({ index, _id }) => ObjectIdHelper.mapIdForClient({
				...items[index],
	 ...
	 * @param {MongoDocument[]} items items
	 * @param {MongoDocument} setOnInsert Default Values for Insert items
	 * @param {object} options An additional options for the multiSave operation
	 ...
	 * @param {boolean} options.rawResponse if true, returns detailed bulkWrite result instead of boolean
	 * @returns {Promise<boolean|MultiUpdateRawResponse>} true or the detailed result if rawResponse is true
	*/
	async multiUpdate(model, operations, options = {}) {
		...
		const bulkItems = operations.map(({ filter, data, options: operationOptions }) => {
			...
			const {
				...
				formattedFilters,
				...extraParams
			} = this.formatDataToUpdate(model, data, operationOptions, filter);
		...
		try {
			...
			const bulkWriteResult = await this.mongo.makeQuery(model, collection => collection
				.bulkWrite(bulkItems, {
					// Unordered only when the detailed response is requested: an ordered bulk stops at the first
					// write error, so the following operations are never executed and can't get an honest verdict
					...rawResponse && { ordered: false },
					...getCommentOption()
				}));

			return rawResponse ? this.formatMultiUpdateRawResponse(operations, bulkWriteResult) : true;

		} catch(err) {

			const failedBulkWriteResult = rawResponse && this.getFailedBulkWriteResult(err);

			if(failedBulkWriteResult)
				return this.formatMultiUpdateRawResponse(operations, failedBulkWriteResult);
	...
	/**
	 * Builds the detailed multiUpdate response from a bulkWrite result, resolved or partial
	 * @param {UpdateOperation[]} operations The operations received by multiUpdate()
	 * @param {import('mongodb').BulkWriteResult} bulkWriteResult The bulkWrite result
	 * @returns {MultiUpdateRawResponse} The detailed result
	 */
	formatMultiUpdateRawResponse(operations, bulkWriteResult) {

		const writeErrors = bulkWriteResult.getWriteErrors();
		const writeConcernError = bulkWriteResult.getWriteConcernError();

		return {
			success: !writeErrors.length && !writeConcernError,
			...
			writeErrors,
			writeConcernErrors: writeConcernError ? [writeConcernError] : [],
			operations: operations.map((operation, index) => {

				const operationErrors = writeErrors.filter(writeError => writeError.index === index);
				...
				return {
					...
					success: !operationErrors.length,
	...
	/**
	 * Gets the partial bulkWrite result of a rejected bulkWrite, only when the server reported write errors.
	 * A driver error raised mid-bulk (connection, timeout) is also wrapped in a MongoBulkWriteError, but it
	 * carries no write error to report, so it must keep being thrown
	 * @param {Error} error The error thrown by bulkWrite()
	 * @returns {import('mongodb').BulkWriteResult|undefined} The partial result, if the error reports write errors
	 */
	getFailedBulkWriteResult(error) {

		if(!(error instanceof MongoBulkWriteError))
			return;

		const { result: bulkWriteResult } = error;

		if(!bulkWriteResult.getWriteErrors().length && !bulkWriteResult.getWriteConcernError())
			return;
	 ...
	 * @param {import('@janiscommerce/model')} model Model instance
	 * @param {object} params Parameters to match documents.
	 * @param {number} params.limit Sets the max amount of matching documents to count. Defaults to count all matching documents. This will be ignored if no filter is provided.
	...
	async getTotals(model, filter, params = {}) {
		...
		const {
			...
			page = 0,
			hint,
			readPreference: totalsReadPreference
		} = model.totalsParams || {};
		...
		try {
			...
			/* istanbul ignore next */
			if(process.env.MONGODB_DEBUG)
				logger.info(`getTotals() ${model.constructor.table} - filters ${JSON.stringify(filters)} - params ${JSON.stringify(params)}`);
			...
			if(length && page > 0 && length < limit)
				...
				const readPreference = params?.readPreference || totalsReadPreference;

				total = await this.mongo.makeQuery(model, collection => collection
					.countDocuments(filters, {
						...getCommentOption(),
						...params?.limit && { limit: params.limit },
						...(hint || params?.hint) && { hint: params.hint || hint },
		...
		try {
			...
			const res = await this.mongo.makeQuery(model, collection => collection
				.findOneAndUpdate(filters, {
					...
					upsert: false,
					returnDocument: 'after', // returns the document after applying the $inc
					includeResultMetadata: true, // driver v6 defaults to returning the document only
					...getCommentOption()
				}));
	...
	async aggregate(model, stages, options = {}) {
		...
		try {
			...
			/* istanbul ignore next */
			if(process.env.MONGODB_DEBUG)
				logger.info(`aggregate() ${model.constructor.table} - filters ${JSON.stringify(mongoStages)} - options ${JSON.stringify(options)}`);

			const res = await this.mongo.makeQuery(model, collection => collection
				.aggregate(mongoStages, {
					...getCommentOption(),
					...options
				}).toArray());
...
/**
 * MongoDB's ObjectId constructor, re-exported for convenience
 * @type {typeof import('mongodb').ObjectId}
 */
