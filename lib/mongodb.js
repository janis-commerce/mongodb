	...
	*/
	prepareFields(fields, toFilter = false) {
		...
		if(typeof fields.id !== 'undefined') {
			...
			else if(Array.isArray(fields.id))
				fields._id = toFilter ? { $in: fields.id.map(id => ObjectID(id)) } : fields.id.map(id => ObjectID(id));
			else if(fields.id instanceof ObjectID)
				fields._id = fields.id;
			else {
				fields._id = Object.entries(fields.id).reduce((acum, [key, value]) => {
					return {
						...acum,
						[key]: Array.isArray(value) ? value.map(id => ObjectID(id)) : ObjectID(value)
					};
				}, {});
			}
