...
module.exports = class MongoDBFilters {
	...
	static get defaultMappers() {
		return {
			dateCreated: value => new Date(value),
			dateCreatedFrom: value => new Date(value),
			dateCreatedTo: value => new Date(value),
			dateModified: value => new Date(value),
			dateModifiedFrom: value => new Date(value),
			dateModifiedTo: value => new Date(value)
		};
	}
	...
	static parseFilterGroup(filterGroup, modelFields) {
		...
		for(const [filterName, filterData] of Object.entries(filterGroup)) {
			...
			const mapper = modelField?.mapper || this.defaultMappers[filterName];
	...
	static getFilterValueObject(filterData, modelField, mapper) {
		...
		return {
			[mongoType]: this.mapValue(mapper, filterData, value)
		};
	}
	...
	static mapValue(mapper, filterData, filterValue) {

		const valueToMap = typeof filterValue !== 'undefined' ? filterValue : filterData;
