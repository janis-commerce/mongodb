...
module.exports = class MongoDBFilters {
	...
	static getFilterValueObject(filterData, modelField, mapper) {
		...
		if(filterData?.raw === true)
			return filterData.value; // when filterData.raw: true received, returned filterData.value without modifications
	...
	static getFilterValue(filterData, modelField) {
		...
		if((filterData.type || modelField.type) === 'caseSensitiveSearch')
			return new RegExp(value);

		if((filterData.type || modelField.type) === 'search')
			// Only add the case insensitive flag if the value contains any letters (including letters with accents)
			return new RegExp(value, (/\p{L}/u).test(value) ? 'i' : undefined);
	...
	static mapToObjectId(value) {
		return Array.isArray(value) ? value.map(v => new ObjectId(v)) : new ObjectId(value);
	}
