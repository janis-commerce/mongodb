...
const { MongoClient, MongoBulkWriteError, ObjectId } = require('mongodb');
...
module.exports.MongoBulkWriteError = MongoBulkWriteError;
		...
		try {
			...
			if(clients[dbConfigKey] instanceof Promise) {
				...
				if(process.env.MONGODB_DEBUG)
					logger.info(`connect() connectionString: ${this.connectionString}`);
